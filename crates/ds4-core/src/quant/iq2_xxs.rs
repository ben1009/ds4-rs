...
//!   * The first uint32_t (low 4 bytes of the 8-byte chunk) holds four 8-bit grid indices.
//!   * The second uint32_t (high 4 bytes) holds four 7-bit sign indices in its low 28 bits and a
//!     4-bit scale multiplier in bits 28–31.
//!
//! Dequantised value:
...
pub fn dequant_block(block: &[u8; BYTES_PER_BLOCK], out: &mut [f32]) {
    ...
    for ib32 in 0..(BLOCK_SIZE / 32) {
        let aux0 = u32::from_le_bytes([
            (qs_u16[ib32 * 4] & 0xFF) as u8,
            (qs_u16[ib32 * 4] >> 8) as u8,
        ...
        for (l, &grid_u64) in grid_vals.iter().enumerate() {
...
mod tests {
    ...
    fn dot_block_matches_dequant_dot_simple() {
        ...
        let dot_ref: f32 = w
            .iter()
            .zip(q8[4..260].iter().map(|&b| b as i8 as f32))
            .map(|(&a, b)| a * b)
            .sum();
